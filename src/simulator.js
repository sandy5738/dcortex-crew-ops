...
const ROSTERS = JSON.parse(fs_1.default.readFileSync(path_1.default.join(DATA_DIR, 'rosters.json'), 'utf-8')).pairings;
const FLIGHTS = JSON.parse(fs_1.default.readFileSync(path_1.default.join(DATA_DIR, 'flights.json'), 'utf-8'));
function getFlightDetails(flightId) {
    return FLIGHTS.find((f) => f.flight_id === flightId);
}
function simulateImpact(crewId, date) {
